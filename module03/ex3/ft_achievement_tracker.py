...
ACHIEVEMENTS_LIST = [
    "First Steps", "Novice Explorer", "Data Miner", "Code Warrior",
    "Logic Master", "Bug Hunter", "Loop Specialist", "Function Guru",
    "List Wizard", "Tuple Traveler", "Set Sentinel", "Exception Expert",
    "Math Magician", "Command Commander", "Analytics Ace", "Quest Champion"
]
...
def main():
    ...
    shared_by_all = set(players["Player 1"]).intersection(*(players[name] for name in player_names[1:]))
    ...
    for name, achs in players.items():
        ...
        others_achs = set().union(*(p_achs for p_name, p_achs in players.items() if p_name != name))
